'use strict';

window.rjs = (function() {
  var keys = [],
      stack = [];

  var cached = {},
      modules = {},
      registry = {};

  var head = document.getElementsByTagName('head')[0];

  var loaded = false;

  function shadow(fn) {
    return function() {
      return fn.apply(null, arguments);
    };
  }

  function ready(next) {
    if (['complete', 'loaded'].indexOf(window.document.readyState) > -1) {
      loaded = true;
      next();
    } else {
      window.document.addEventListener('DOMContentLoaded', function() {
        loaded = true;
        next();
      }, false);
    }
  }

  function fetch(id, ref) {
    if (!modules[id]) {
      modules[id] = ref[2].apply(null, ref[1].map(function(dep) {
        return modules[dep];
      }));

      if (ref[0] && ref[0] !== id) {
        modules[ref[0]] = modules[id];
      }
    }
  }

  function push(id) {
    if (!registry[id]) {
      throw new Error('missing definition for `' + id + '`');
    }

    return function(next) {
      var script = document.createElement('script');

      script.type = 'text/javascript';
      script.src = registry[id];
      script.async = false;

      head.appendChild(script);

      script.onreadystatechange = script.onload = function() {
        head.removeChild(script);
        cached[id] = true;
        next();
      };
    };
  }

  function load(deps) {
    deps.forEach(function(dep) {
      if (!(modules[dep] || cached[dep])) {
        stack.push(push(dep));
        keys.push(dep);
      }
    });
  }

  function call(fn, deps, extra) {
    var args = [];

    deps.forEach(function(dep) {
      if (modules[dep]) {
        args.push(modules[dep]);
      }
    });

    return fn.apply(null, args.concat(extra.filter(function(dep) {
      return dep;
    })));
  }

  function run(all, done) {
    function next() {
      var task = all.shift();

      if (!task) {
        return done();
      }

      task(next);
    }

    next();
  }

  function add(ref, id, deps, factory) {
    if (!ref) {
      throw new Error('indeterminate definition for `' + factory + '`');
    }

    if (!deps) {
      fetch(ref, [id, [], factory]);
      return;
    }

    load(deps);

    stack.push(function(next) {
      fetch(ref, [id, deps, factory]);
      next();
    });
  }

  function def(id, deps, factory) {
    if (typeof id === 'function') {
      factory = id;
      id = null;
    }

    if (Array.isArray(id)) {
      factory = deps;
      deps = id;
      id = null;
    }

    if (typeof deps === 'function') {
      factory = deps;
      deps = null;
    }

    var ref = keys.shift();

    add(ref, id, deps, factory);
  }

  function req(deps, callback) {
    if (!loaded) {
      stack.unshift(ready);
    }

    var fixed = [],
        extra = [];

    function append(data) {
      if (data) {
        extra.push(data);
      }
    }

    deps.forEach(function(dep) {
      if (typeof dep === 'function') {
        stack.push(function(next) {
          if (dep.length) {
            dep(function(data) {
              append(data);
              next();
            });
          } else {
            var retval = dep();

            if (retval && typeof retval.then === 'function') {
              retval.then(function(data) {
                append(data);
                next();
              });
            } else {
              append(retval);
              next();
            }
          }
        });
      } else {
        fixed.push(dep);
      }
    });

    load(fixed);

    run(stack, function() {
      call(callback, fixed, extra);
    });
  }

  function set(src, name, factory) {
    if (typeof name === 'string') {
      registry[name] = src;
      return;
    }

    if (typeof name === 'function') {
      factory = name;
      name = [];
    }

    add(src, null, name, factory);
  }

  // not standard yet!
  window.define = shadow(def);
  window.define.amd = false;

  return {
    define: shadow(set),
    require: shadow(req)
  };
})();
